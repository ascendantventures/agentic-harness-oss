...
export class ProvisionStation extends BaseStation {
    ...
    concurrency = 2;
    ttl = 600000; // 10 min — API calls, not agent work
     ...
     */
    async directRun(issue, ctx) {
        const log = (msg) => ctx.log(`[PROVISION] #${issue.number}: ${msg}`);
        const token = process.env.SUPABASE_MANAGEMENT_API_TOKEN;
        const orgId = process.env.SUPABASE_ORG_ID;
    ...
    async findExistingProject(token, orgId, name, log) {
        try {
    ...
    async createProject(token, orgId, name, dbPassword, log) {
        try {
            const res = await fetch(`${SUPABASE_API}/projects`, {
                method: 'POST',
                headers: {
    ...
    async getProjectKeys(token, projectId, log) {
        try {
    ...
    async runMigrations(token, projectId, buildRepo, ctx, log) {
        try {
            // Fetch migration files from GitHub
            const migrationList = execSync(`gh api repos/${buildRepo}/contents/supabase/migrations --jq '.[].name' 2>/dev/null`, { encoding: 'utf8', timeout: 15000 }).trim().split('\n').filter(Boolean).sort();
            ...
            for (const filename of migrationList) {
                try {
                    const sql = execSync(`gh api repos/${buildRepo}/contents/supabase/migrations/${filename} --jq '.content' 2>/dev/null | base64 -d`, { encoding: 'utf8', timeout: 15000 }).trim();
                    if (!sql)
                        continue;
    ...
    // ─── Vercel helpers ───────────────────────────────────────────────────────
    async injectVercelEnvVars(token, projectName, keys, log) {
    ...
    async triggerRedeploy(token, projectName, log) {
        try {
    ...
    commentSuccess(issueNumber, ctx, data) {
        const body = [
    ...
    // ─── Utilities ─────────────────────────────────────────────────────────────
    extractBuildRepo(issue, ctx) {
        try {
            const result = execSync(`gh issue view ${issue.number} --repo ${ctx.env.repo} --comments --json comments`, { encoding: 'utf8', timeout: 15000 });
