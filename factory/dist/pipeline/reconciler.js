 ...
 */
export function checkArtifactGate(issueNumber, station, repo, buildRepo, log) {
    ...
    let changedFiles = [];
    try {
        const filesOutput = execSync(`gh pr view "${branchName}" --repo ${buildRepo} --json files --jq '[.files[].path]'`, { encoding: 'utf8', timeout: 15000 }).trim();
 ...
 */
function bounceForArtifactGate(issueNumber, station, repo, stageLabel, missing, log) {
    const body = [
    ...
    try {
        execSync(`gh issue comment ${issueNumber} --repo ${repo} --body ${JSON.stringify(body)}`, { encoding: 'utf8', timeout: 15000 });
...
export function flipLabel(issueNumber, repo, currentLabel, nextLabel, log, reason) {
    try {
        ...
        recordStationTransition(issueNumber, toStation, fromStation, supabaseUrl, supabaseKey, log).catch(() => { });
        return true;
    }
    catch (e) {
        ...
        if (atInputLabel || atOutputLabel) {
            ...
            if (hasArtifact) {
                ...
                if (ARTIFACT_GATES[station]) {
                    ...
                    let buildRepo = repo;
                    try {
                        const bodyJson = execSync(`gh issue view ${issueNumber} --repo ${repo} --json body --jq '.body'`, { encoding: 'utf8', timeout: 15000 }).trim();
    ...
    try {
        ...
        for (const issue of issues) {
            ...
            if (stationLabels.length > 1) {
                ...
                const stageOrder = ['station:intake', 'station:spec', 'station:design', 'station:build', 'station:provisioned', 'station:qa', 'station:uat', 'station:bugfix', 'station:done'];
